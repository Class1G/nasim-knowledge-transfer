...
# evaluation of every model trained under each mask type on FULL, and afterwards evaluated under each mask in runtime on FULL
# totals a 4 x 4 style grid of every mask combination ( train x runtime )
...
MODEL_DIR =     "trained_agents/action_masking"
RESULTS_DIR =   "results/actionmasking_eval"
...
for model_type in ["none", "nodupe", "poss", "adv"]:
    
    print(f"MODEL - MASK IN TRAINING: {model_type}")
    print()
    
    rs_results_path = f"{RESULTS_DIR}/AM_results_{model_type}.csv"

    with open(rs_results_path, "w", newline = "") as f:
        writer = csv.writer(f)
        writer.writerow(["am_post_type", "learning_rate", "ent_coefs", "mean_overall", "steps_overall", "goal_rate",
            ...
            for l in learning_rates:
                for e in ent_coefs:
                    ...
                    if model_type == "none":
                        ...
                        model_name = f"mppo_AM_{model_type}_L{str(l)}_E{e}"
