...
from mask_wrapper import ActionMasking_none_PART as wrapper



# unmodified train on PART
...
for l in learning_rates:
    for e in ent_coefs:
            ...
            model = MaskablePPO('MlpPolicy',
                        ...
                        ent_coef =          e,
                        clip_range =        c,
                        tensorboard_log =   f"./logs/base_part/{model_name}", verbose = 0, device = DEVICE)

            model.learn(total_timesteps = TOTALSTEPS, tb_log_name = model_name, progress_bar = True)
            model.save(f"trained_agents/base_part/{model_name}")
            vec_env.close()